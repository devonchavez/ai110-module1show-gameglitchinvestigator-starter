...
# FIX: Added check to reset game state when difficulty changes, ensuring secret is within new range.
...
# FIXME: Logic breaks here when a difficulty is selected the range for the difficulty is not updated.
# FIX: Changed random.randint(1, 100) to random.randint(low, high) to use current difficulty range.
if new_game:
    st.session_state.attempts = 0
    st.session_state.secret = random.randint(low, high)
